...
import { MATURITY_BLOCKS, UNLOCK_TIME_TIMESTAMP_THRESHOLD, WALLET_ADDRESS_PREFIX_STRING } from './config';
...
export class TransactionPicker {
    constructor(outputs, currentHeight, maturityBlocks = MATURITY_BLOCKS, walletPublicKey = '', walletPrivateKey = '') {
     ...
     */
    pickInputs(targetAmount, fee = DEFAULT_FEE) {
     ...
     */
    pickInputsForDestinations(destinations, fee = DEFAULT_FEE) {
...
export class TransactionSerializer {
     ...
     * }
     */
    static serializeKeyInput(input) {
        const parts = [];
     ...
     */
    static serializeExtra(publicKey) {
        const parts = [];
     ...
     */
    static serializeTransactionPrefix(inputs, outputs, publicKey, unlockTime = 0, version = 1) {
        const parts = [];
        // version (varint)
     ...
     */
    static generateSignature(prefixHash, publicKey, privateKey) {
...
export class TransactionBuilder {
     ...
     */
    buildTransaction(pickResult, publicKey) {
        // Build KeyInput array from selected inputs
        const inputs = pickResult.inputs.map((selected) => {
            const input = {
        ...
        const txBytes = TransactionSerializer.serializeTransaction(inputs, outputs, publicKey, 0, // unlockTime
        1 // version
        );
 ...
 */
export function createTransaction(outputs, destinations, publicKey, privateKey, currentHeight, node, maturityBlocks = MATURITY_BLOCKS, fee = DEFAULT_FEE) {
 ...
 */
export async function sendTransaction(txHex, node) {
 ...
 */
export async function sendFromMnemonic(request) {
    // Derive keys from mnemonic
    const { publicKey, privateKey } = deriveKeysFromMnemonic(request.mnemonic);
    // Create transaction
    const builtTx = createTransaction(request.outputs, request.destinations, publicKey, privateKey, request.currentHeight, request.node, request.maturityBlocks ?? MATURITY_BLOCKS, request.fee ?? DEFAULT_FEE);
    if (!builtTx) {
