 ...
 */
export function classifyTransaction(txHash, data, ctx) {
    ...
    const status = confirmations >= MATURITY_BLOCKS ? 'confirmed' : 'pending';
 ...
 */
export function extractFromToAddresses(txHash, type, walletOutputs, ctx) {
    ...
    if (type === 'coinbase') {
        ...
        const to = deduplicateAddresses(walletOutputs.map(o => ({
    ...
    if (!rawOutputs || rawOutputs.length === 0) {
        ...
        const from = deduplicateAddresses(Array.from(walletAddressesSet).map(addr => ({ address: addr })));
    ...
    const to = deduplicateAddresses(Array.from(externalAddresses).map(addr => ({ address: addr })));
 ...
 */
export function createDisplayTransaction(hash, type, amount, blockHeight, timestamp, currentHeight, walletOutputs = [], ctx) {
 ...
 */
export function getTransactions(allOutputs, allSpends, stakingTxHashes, publicKeys, currentHeight, rawTransactionOutputs, limit = Number.MAX_SAFE_INTEGER) {
    const ctx = {
        publicKeys,
        currentHeight,
    ...
    for (const [txHash, data] of txMap) {
        ...
        if (data.isCoinbase && data.outputs.length === 2) {
            ...
            if (amount0 < 1000000 && amount1 >= 100000000) {
                transactions.push(createDisplayTransaction(txHash, 'coinbase', totalIncoming, data.blockHeight, data.timestamp, currentHeight, data.outputs, ctx));
            }
            // New structure with staking: large block reward + smaller staking reward
                ...
                transactions.push(createDisplayTransaction(`${txHash}_mining`, 'coinbase', amount0, data.blockHeight, data.timestamp, currentHeight, [data.outputs[0]], ctx));
                transactions.push(createDisplayTransaction(`${txHash}_staking`, 'staking', amount1, data.blockHeight, data.timestamp, currentHeight, [data.outputs[1]], ctx));
            }
            else {
                transactions.push(createDisplayTransaction(txHash, 'coinbase', totalIncoming, data.blockHeight, data.timestamp, currentHeight, data.outputs, ctx));
            }
