...
function calculateChecksumWord(words) {
    let trimmed = '';
...
function uint8BeToBigInt(buffer, offset, size) {
    if (size < 1 || size > 8) {
...
function generateEd25519Keypair(privateKeyBytes) {
    let scalarBigInt = 0n;
    for (let i = 0; i < 32; i++) {
...
export class PastellaWallet {
    ...
    static async importFromMnemonic(mnemonic) {
    ...
    static async importFromPrivateKey(privateKeyHex) {
 ...
 */
export function derivePublicKeyFromMnemonic(mnemonic) {
    if (!verifyMnemonicChecksum(mnemonic)) {
        throw new Error('Invalid mnemonic checksum');
    }
    const privateKeyBytes = mnemonicToPrivateKey(mnemonic);
 ...
 * Returns the public key as a hex string for wallet sync
 */
export function derivePublicKeyFromPrivateKey(privateKeyHex) {
    // Remove 0x prefix if present
    const hexKey = privateKeyHex.replace(/^0x/i, '');
    // Validate hex length
    if (hexKey.length !== 64) {
 ...
 */
export function coinsToAtomic(coins) {
 ...
 */
export function atomicToCoins(atomic) {
...
export { WalletSync } from './walletSync';
