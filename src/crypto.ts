 ...
 */
function scReduce(input: Uint8Array): Uint8Array {
  if (input.length < 64) {
 ...
 */
export function generateSchnorrSignature(
 ...
 */
function scalarToBigInt(scalar: Uint8Array): bigint {
  let num = 0n;
  for (let i = 0; i < 32; i++) {
 ...
 */
function bigIntToScalar(num: bigint): Uint8Array {
  const result = new Uint8Array(32);
  for (let i = 0; i < 32; i++) {
 ...
 */
export function generateKeyImage(publicKey: string, privateKey: string): string {
  // Convert keys to bytes
  const publicKeyBytes = hexToBytes(publicKey);
  const privateKeyBytes = hexToBytes(privateKey);
