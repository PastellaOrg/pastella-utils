 ...
 */
export function calculateUnlockTime(lockDurationDays: number, currentHeight: number): number {
 ...
 */
export function getStakingDenominations(amount: number): number[] {
  return [amount];
}
 ...
 */
export function findPreciseStakingOutputsTxHash(
  amount: number,
  outputs: WalletOutput[],
  currentHeight: number,
  currentFee: number = MIN_FEE,
  ...
  const feeOutput = spendable.find(o =>
    o.amount === currentFee &&
 ...
 */
export function hasPreciseStakingOutputs(
  amount: number,
  outputs: WalletOutput[],
  currentHeight: number,
  currentFee: number = MIN_FEE,  // Use dynamic fee
  maturityBlocks: number = MATURITY_BLOCKS
): boolean {
  // Get spendable outputs
  const matureHeight = currentHeight - maturityBlocks;
 ...
 */
export function pickStakingInputs(
  amount: number,
  outputs: WalletOutput[],
  currentHeight: number,
  publicKey: string,
  ...
  currentFee: number = MIN_FEE,  // Use dynamic fee
  maturityBlocks: number = MATURITY_BLOCKS,
  preparationTxHash?: string  // NEW: Only use outputs from preparation transaction
): { inputs: SelectedInput[], totalInput: number } | null {
  // Get spendable outputs
  const matureHeight = currentHeight - maturityBlocks;
  const spendable = outputs.filter(o => {
    const isUnspent = !o.spentHeight;
  ...
  let amountInput: WalletOutput | null = null;
  ...
  let feeInput: WalletOutput | null = null;
...
export interface TransactionExtraStaking {
  stakingType: number;         // 101 = STAKING_TX_TYPE
  amount: number;              // Amount being staked
  unlockTime: number;          // When funds can be unlocked
  lockDurationDays: number;    // Lock period in days
  signature: string;           // 64-byte hex signature proving ownership
}

/**
 * Create staking extra field with signature
 */
export function createStakingExtra(
  amount: number,
  unlockTime: number,
 ...
 */
export function generateStakingSignature(
  amount: number,
  lockDurationDays: number,
  unlockTime: number,
  publicKey: string,
  ...
  const signature = generateSchnorrSignature(
    messageHashBytes,
    publicKey,
    privateKey
  );
 ...
 */
export async function prepareStakingOutputs(
  amount: number,
  address: string,
  outputs: WalletOutput[],
  publicKey: string,
  ...
  try {
    ...
    let selectedInputs: WalletOutput[] = [];
    let totalInput = 0;
    ...
    const inputs: SelectedInput[] = selectedInputs.map(output => ({
      output,
    ...
    const txHex = await buildPreparationTransactionHex(
      amount,
...
export async function createStakingTransaction(
  ...
  node: NodeConfig,
  maturityBlocks: number = MATURITY_BLOCKS,
  preparationTxHash?: string  // NEW: Use outputs from this specific transaction
): Promise<StakingResult> {
  try {
    // Use MIN_FEE constant
    const currentFee = MIN_FEE;
    ...
    const signature = generateStakingSignature(
      amount,
      lockDurationDays,
      unlockTime,
      publicKey,
    ...
    const txHex = await buildStakingTransactionHex(
      amount,
      lockDurationDays,
      unlockTime,
      signature,
 ...
 */
async function buildPreparationTransactionHex(
  amount: number,
  currentFee: number,
  ...
  const serializedOutputs: SerializedKeyOutput[] = [
  ...
  const prefixBytes = serializeTransactionPrefixWithCustomExtra(
    serializedInputs,
    serializedOutputs,
    extraData,
    0,  // unlockTime = 0 (normal transaction)
    1   // version = 1
  );
  ...
  for (const input of inputs) {
    const sig = generateSchnorrSignature(
      prefixHashBytes,
      input.publicKey,
      input.privateKey
    );
  ...
  const txParts: Uint8Array[] = [prefixBytes, ...signatures];
 ...
 */
function buildNormalExtra(publicKey: string): Uint8Array {
  const extra: number[] = [];
 ...
 */
async function buildStakingTransactionHex(
  amount: number,
  lockDurationDays: number,
  unlockTime: number,
  signature: string,
 ...
 */
function buildExtraWithStaking(
  publicKey: string,
  amount: number,
  unlockTime: number,
  lockDurationDays: number,
 ...
 */
function serializeTransactionPrefixWithCustomExtra(
...
function createSerializationHelpers() {
  ...
  const readVarint = (bytes: Uint8Array, offset: number): { value: number; bytesRead: number } => {
    let value = 0n;
    let shift = 0n;
    let bytesRead = 0;
 ...
 */
function serializeKeyInput(input: SerializedKeyInput): Uint8Array {
  const parts: Uint8Array[] = [];
  const { writeVarint } = createSerializationHelpers();
 ...
 */
function serializeTransactionOutput(output: SerializedKeyOutput): Uint8Array {
  const parts: Uint8Array[] = [];
  const { writeVarint } = createSerializationHelpers();
 ...
 */
function writeVarintLocal(value: number): number[] {
  const bytes: number[] = [];
  const bigValue = BigInt(value);
  const mask = 0x7fn;
 ...
 */
export function createPendingStake(
  amount: number,
  lockDurationDays: number,
  address: string,
  state: PendingStakeState = PendingStakeState.PREPARING
): PendingStake {
  return {
 ...
 */
export function updatePendingStake(
  stake: PendingStake,
  updates: Partial<Omit<PendingStake, 'id' | 'createdAt' | 'amount' | 'lockDurationDays' | 'address'>>
): PendingStake {
 ...
 */
export function checkOutputsAvailable(
  stake: PendingStake,
  outputs: WalletOutput[],
  currentHeight: number,
  currentFee: number = MIN_FEE,
