...
import { WalletOutput, WalletSyncState } from './types';
 ...
 */
export function recalculateBalances(ctx: BalanceContext): { available: number; locked: number; stakingLocked: number } {
  ...
  for (const output of outputs.values()) {
    ...
    const unlockMet = isOutputSpendable(
      output.blockHeight,
      output.unlockTime,
      0, // blockTimestamp not stored
      currentHeight
    );
 ...
 */
export function getStakingLockedBalance(ctx: BalanceContext): number {
 ...
 * Get unspent outputs
 */
export function getUnspentOutputs(allOutputs: WalletOutput[]): WalletOutput[] {
 ...
 */
export function getLockedOutputs(
  allOutputs: WalletOutput[],
  stakingTxHashes: Set<string>,
  currentHeight: number
): WalletOutput[] {
  const locked = getUnspentOutputs(allOutputs).filter(o => {
    ...
    const spendable = isOutputSpendable(
      o.blockHeight,
      o.unlockTime,
      0, // blockTimestamp not stored
      currentHeight
