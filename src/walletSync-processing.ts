...
export async function processBlock(
  block: WalletBlockInfo,
  ctx: ProcessingContext,
  syncedBlocks: Map<number, SyncedBlockInfo>,
  blockCheckpoints: Map<number, string>,
  onBlockProcessed?: BlockProcessedHandler,
  onTransactionFound?: TransactionFoundHandler,
  onSpendFound?: SpendFoundHandler,
  onTransactionDiscovered?: TransactionDiscoveredHandler
): Promise<{ newOutputs: WalletOutput[]; newSpends: WalletSpend[] }> {
  // Check for fork
  ...
  const syncedBlock: SyncedBlockInfo = {
 ...
 */
export function processOutputs(block: WalletBlockInfo, ctx: ProcessingContext): WalletOutput[] {
  const { publicKeys, currentHeight, outputs } = ctx;
  const newOutputs: WalletOutput[] = [];
 ...
 */
function processTransactionSpends(
  transactions: any[],
  block: WalletBlockInfo,
  ctx: ProcessingContext,
  ...
  for (const tx of transactions) {
    ...
    for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
      ...
      if (!inputAmount) continue;
      processedInputs.add(inputId);

      let matchedOutput: { key: string; output: WalletOutput } | null = null;
      ...
      if (input.transactionHash && input.outputIndex !== undefined) {
        for (const [key, output] of outputs) {
          if (output.spentHeight) continue;
      ...
      if (!matchedOutput) {
        const matchingOutputs: Array<{key: string, output: WalletOutput}> = [];
 ...
 */
export function processSpends(block: WalletBlockInfo, ctx: ProcessingContext): WalletSpend[] {
  const { outputs, spends } = ctx;
  const newSpends: WalletSpend[] = [];
 ...
 */
export function pruneSpentInputs(currentHeight: number, outputs: Map<string, WalletOutput>): void {
 ...
 */
export function addCheckpoint(height: number, hash: string, blockCheckpoints: Map<number, string>): void {
 ...
 */
export function pruneCheckpoints(blockCheckpoints: Map<number, string>): void {
 ...
 */
export function pruneSyncedBlocks(syncedBlocks: Map<number, SyncedBlockInfo>): void {
 ...
 */
function trackPendingTransactions(
